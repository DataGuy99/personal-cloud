...
## White Paper v1.2 | 2026-06-20

---

## 1. EXECUTIVE SUMMARY

Build a headless, self-hosted file server on a Lenovo ThinkCentre M920q micro-PC that:
...
The M920q has one M.2 NVMe slot, one 2.5" bay, and one proprietary PCIe connector. With the 01AJ940 riser card, the PCIe connector becomes a standard PCIe 3.0 x8 low-profile slot. The riser occupies the 2.5" bay space (one or the other, not both). Boot from the included NVMe, use the PCIe slot for an LSI 9207-8i HBA providing 8 SATA ports.

| Component | Spec | Source | Cost |
|-----------|------|--------|------|
| **Mini PC** | Lenovo ThinkCentre M920q (i5-8500T, 8GB, 256GB NVMe) | eBay (northtexassellers) | $139.99 |
| **CPU** | Intel Core i5-8500T (6C/6T, 2.10 GHz base / 3.50 GHz boost) | Included | -- |
| **RAM** | 8GB DDR4-2666 (1x8GB, expandable to 64GB) | Included | -- |
| **Boot Storage** | 256GB NVMe M.2 (included with unit) | Included | -- |
| **PCIe Riser** | 01AJ940 PCIE16 expansion riser for M920q | Amazon (BestParts) | $21.99 |
| **HBA Card** | LSI 9207-8i PCIe 3.0 x8, IT mode, 2x SFF-8087 | eBay | ~$35 |
| **SAS Breakout Cables** | 2x SFF-8087 to 4x SATA (included with HBA) | Included | -- |
| **Initial Data Drive** | Micron 1300 256GB 2.5" SATA SSD | eBay (harddrivesonly) | $24.99 |
| **Drive Housing** | Custom 3D-printed 10-inch rack with drive caddies | Self-printed | ~$10 filament |
| **Cooling** | Rack-mounted fans | Amazon | ~$10 |
...
|                    ThinkCentre M920q                       |
|  +---------------+  +----------------------------------+  |
|  | 256GB NVMe    |  | PCIe Riser (01AJ940)             |  |
|  | (M.2 slot)    |  |   +-- LSI 9207-8i HBA            |  |
|  | OS Boot       |  |       +-- 2x SFF-8087 out         |  |
|  +---------------+  +----------------+-----------------+  |
|         |                             |                    |
|         v                             v                    |
|    Debian Server            3D-Printed 10" Rack            |
|    /boot, /root             Micron 256GB SSD +             |
|    copyparty                3.5"/2.5" HDDs/SSDs            |
|    Jellyfin                 via SATA through HBA           |
|    WireGuard                ATX PSU for drive power        |
+-----------------------------------------------------------+
```

### 2.2a Boot NVMe Health Verification (Post-Install)

The included NVMe is an OEM pull. Verify on first boot:
...
- **Drives:** Mixed sizes accepted -- 256GB SSD, 2TB HDD, 4TB HDD, etc.
- **Expansion:** Add drive -> format ext4 -> connect to HBA SATA port -> add to MergerFS mount -> done
- **Capacity:** Up to 8 SATA drives via HBA (2x SFF-8087 breakout cables)
- **No RAID:** Single-drive failure = lose only that drive's contents
- **Future parity:** Can add SnapRAID later without rebuilding pool

### 2.4 3D-Printed 10-Inch Rack
- **Frame:** 3D-printed 10-inch server rack
- **Compute shelf:** 1U or 2U adapter for the M920q
- **Drive bays:** 4x 2.5" and 2x 3.5" caddies
- **Networking:** Shelf for modem + router
- **Cooling:** Rack-mounted fans for airflow
- **Cable management:** SFF-8087 breakout cables + SATA power routed through rack
- **Power:** ATX PSU mounted at rack base, provides SATA power to all drives
...
| **Boot** | Legacy BIOS or UEFI | M920q supports both |
...
- **Transcoding:** Intel Quick Sync (i5-8500T UHD Graphics 630)
...
Router (WiFi off 1-7 AM) ──► ThinkCentre (WiFi disabled, no route)
     │
     └── Modem (always on) ──► ThinkCentre (second Ethernet adapter, updates only)
```

**Note:** The M920q has a single Ethernet port. A second wired connection to the modem requires a USB Ethernet adapter (~$12).
...
table inet update_filter {
    chain output {
        ...
        meta hour "01:00:00"-"06:59:59" oifname "enp*" tcp dport 443 accept
        meta hour "01:00:00"-"06:59:59" oifname "enp*" udp dport 53 accept
...
**Note on `oifname "enp*"`:** second Ethernet adapters on Linux typically get interface names starting with `enx` (based on MAC address). Verify with `ip link` after plugging in. The primary onboard NIC is typically `enp0s31f6` or similar on the M920q.
...
| **TOTAL** | **~3-5%** | **Unnoticeable on i5-8500T 6C/6T for 5 clients** |
...
- [ ] ~~Order ThinkCentre M920q ($69.99)~~ **DONE**
- [ ] ~~Order 256GB NVMe ($19.99)~~ **DONE**
- [ ] Order PCIe riser (01AJ940) and LSI 9207-8i HBA
- [ ] Order USB Ethernet adapter for wired update fallback (~$12, optional)
- [ ] Print drive housing (or use temporary external USB dock)
- [ ] Install included NVMe in M.2 slot, 500GB HDD stays in 2.5" bay
...
| Item | Cost |
|------|------|
| ThinkCentre M920q (i5-8500T, 8GB, 500GB HDD) | $69.99 |
| 256GB NVMe (boot) | $19.99 |
| LSI 9207-8i HBA + cables | ~$35 |
| 3D-printed housing (filament) | ~$5 |
| 120mm fan + buck converter | ~$10 |
| ATX PSU for drives | ~$15 |
| second Ethernet adapter (optional, for update fallback) | ~$12 |
| **Phase 1 Total** | **~$140-152** |
| Future: 4TB HDD (on sale) | ~$60 |
| Future: Additional drives + HBA cable | ~$70 each |
...
| CPU insufficient for 5 streams | Low | Low | i5-8500T 6C/6T with UHD 630 Quick Sync handles transcoding well |
